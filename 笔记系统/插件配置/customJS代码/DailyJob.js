...
`;
    ...
    chronos(tfile,with_head=true) {
        ...
        if(!Array.isArray(tfile)){
            ...
            events += jobs.map((job) => {
                let date = '2000-01-01'+'T'+job.st.format('HH:mm:ss');
                let et = '2000-01-01'+'T'+job.st.clone().add(job.xt,'minutes').format('HH:mm:ss');
                if(et.localeCompare(date)<0){
                    et = '2000-01-02'+'T'+job.st.clone().add(job.xt,'minutes').format('HH:mm:ss');
                }
                return `- [${date}~${et}] {${tfile.basename}} ${job.do} ${job.xt}`;
            })
            .join("\n");
        }else{
            for(let t of tfile){
                events +=this.chronos(t,false)+'\n';
            }
            events = events.trim()
        }
    ...
    chronos_of_days(begt=7,endt=2){
        let tfile = this.nc.chain.get_last_daily_note();
        let tfiles = this.nc.chain.get_chain(tfile,begt,endt,true);
        return this.chronos(tfiles);
    }
