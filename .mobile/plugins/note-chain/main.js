...
var NoteChain = class {
  ...
  get_recent_tfiles(only_md = true) {
    ...
    if (files && files.length > 0) {
      recent = files.map((x) => this.get_tfile(x)).filter((x) => x);
    }
    ...
    if (only_md) {
      recent = recent.filter((x) => x.extension == "md");
    }
    return recent;
  }
...
var WordCount = class {
  ...
  async set_mtime_value(tfile, key, val) {
    ...
    await this.app.fileManager.processFrontMatter(
      tfile,
      (fm) => {
        let t = import_obsidian5.moment.unix(tfile.stat.mtime / 1e3);
        let mtime = t.format("YYYY-MM-DD");
        if (fm[key] == null) {
          if (val > 0) {
            fm[key] = {};
            if (mtime == (0, import_obsidian5.moment)().format("YYYY-MM-DD") && mtime != import_obsidian5.moment.unix(tfile.stat.ctime / 1e3).format("YYYY-MM-DD")) {
      ...
      if (editor) {
        ...
        if (editorState.selection && editorState.sanchor && editorState.shead) {
          try {
            ...
            new import_obsidian5.Notice(`Error setting selection:${error}`, 3e3);
          }
  ...
  get_new_words(tfile, day = (0, import_obsidian5.moment)().format("YYYY-MM-DD")) {
    var _a;
    let meta = this.app.metadataCache.getFileCache(tfile);
  ...
  async update_word_count_of_vault() {
    ...
    for (let tfile of tfiles) {
      new import_obsidian5.Notice(`${i}/${tfiles.length}:${tfile.name}`, 3e3);
  ...
  sum_words_of_tifles(files, begt, endt) {
    if (typeof begt == "number") {
      begt = (0, import_obsidian5.moment)().add(-begt, "days").format("YYYY-MM-DD");
    }
    if (typeof endt == "number") {
      endt = (0, import_obsidian5.moment)().add(-endt, "days").format("YYYY-MM-DD");
    }
        ...
        for (let [relation, linkedNote] of Object.entries(links)) {
          if (linkedNote instanceof Array) {
            for (let item of linkedNote) {
              ...
              if (linkedTFile instanceof import_obsidian6.TFile) {
...
var CanvasGraph = class {
  ...
  gen_node(NN, tfile, x = 0, y = 0, width = 400, height = 400) {
    ...
    if (typeof tfile == "string") {
      ...
      rsp["type"] = "file";
      if (tfile instanceof import_obsidian6.TFolder) {
...
var NCSettingTab = class extends import_obsidian7.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian7.Setting(containerEl).setName(this.plugin.strings.setting_isSortFileExplorer).addToggle(
    ...
    new import_obsidian7.Setting(containerEl).setName(this.plugin.strings.setting_isFolderFirst).addToggle(
    ...
    new import_obsidian7.Setting(containerEl).setName(this.plugin.strings.setting_PrevChain).addText((text) => text.setValue(this.plugin.settings.PrevChain).onChange(async (value) => {
      this.plugin.settings.PrevChain = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian7.Setting(containerEl).setName(this.plugin.strings.setting_suggesterNotesMode).addDropdown((dropdown) => dropdown.addOption("item_get_brothers", this.plugin.strings.item_get_brothers).addOption("item_uncle_notes", this.plugin.strings.item_uncle_notes).addOption("item_notechain", this.plugin.strings.item_notechain).addOption("item_same_folder", this.plugin.strings.item_same_folder).addOption("item_inlinks_outlinks", this.plugin.strings.item_inlinks_outlinks).addOption("item_inlins", this.plugin.strings.item_inlins).addOption("item_outlinks", this.plugin.strings.item_outlinks).addOption("item_all_noes", this.plugin.strings.item_all_noes).addOption("item_recent", this.plugin.strings.item_recent).addOption("", "").setValue(this.plugin.settings.suggesterNotesMode).onChange(async (value) => {
      this.plugin.settings.suggesterNotesMode = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian7.Setting(containerEl).setName(this.plugin.strings.setting_NextChain).addText((text) => text.setValue(this.plugin.settings.NextChain).onChange(async (value) => {
      this.plugin.settings.NextChain = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian7.Setting(containerEl).setName(this.plugin.strings.setting_auto_notechain).addToggle(
      (text) => text.setValue(this.plugin.settings.auto_notechain).onChange(async (value) => {
        this.plugin.settings.auto_notechain = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian7.Setting(containerEl).setName(this.plugin.strings.setting_refreshDataView).addToggle(
      (text) => text.setValue(this.plugin.settings.refreshDataView).onChange(async (value) => {
        this.plugin.settings.refreshDataView = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian7.Setting(containerEl).setName(this.plugin.strings.setting_refreshTasks).addToggle(
      (text) => text.setValue(this.plugin.settings.refreshTasks).onChange(async (value) => {
        this.plugin.settings.refreshTasks = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian7.Setting(containerEl).setName(this.plugin.strings.setting_wordcout).addToggle(
      (text) => text.setValue(this.plugin.settings.wordcout).onChange(async (value) => {
        this.plugin.settings.wordcout = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian7.Setting(containerEl).setName(this.plugin.strings.setting_wordcout_xfolder).addTextArea(
      (text) => text.setValue(this.plugin.settings.wordcountxfolder).onChange(async (value) => {
        this.plugin.settings.wordcountxfolder = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian7.Setting(containerEl).setName(this.plugin.strings.setting_avata).addTextArea(
    ...
    if (tfile) {
      ...
      if (key) {
        ...
        if (res && !(res === note.basename) && !(res === "")) {
          ...
          if (dst) {
            new import_obsidian8.Notice("Exist:" + res + note.extension, 3e3);
          } else {
...
// main.ts
var NoteChainPlugin = class extends import_obsidian9.Plugin {
  async onload() {
    this.debug = true;
          ...
          menu.addItem((item) => {
            item.setTitle(this.strings.filemenu_create_next_note).setIcon("file-plus").onClick(async () => {
              ...
              if (this.chain.get_tfile(dst)) {
                new import_obsidian9.Notice("Exists:" + file.path, 3e3);
              } else {
            ...
            item.setTitle(this.strings.filemenu_move_as_next_note).setIcon("hand").onClick(async () => {
              ...
              if (notes) {
                let anchor = await this.chain.tp_suggester(
                  (f) => f.name,
                  notes.filter((x) => x instanceof import_obsidian9.TFile)
                );
