...
~~~`, "g");
  ...
  async get_code_section(tfile, ctype = "", idx = 0, as_simple = true) {
    ...
    let sections = (_a = dvmeta == null ? void 0 : dvmeta.sections) == null ? void 0 : _a.filter((x) => x.type == "code").filter((x) => {
      ...
      return c2.startsWith("```" + ctype) || c2.startsWith("~~~" + ctype);
    });
  ...
  async get_heading_section(tfile, heading, idx = 0, with_heading = true) {
    ...
    if (sections.length == 1) {
      ...
      const choices = sections.map(
        (x) => {
          var _a2;
          return this.get_heading_ctx(ctx, (_a2 = dvmeta.headings) != null ? _a2 : [], x, with_heading);
        }
      );
...
var BaseWebViewer = class {
  ...
  async open_homepage(url2 = this.homepage, idx = 0) {
    let views = this.get_webviews(url2);
    if (idx == -1) {
      idx = views.length;
    }
    let n = views.length;
    while (n < idx + 1) {
      ...
      if (plugin) {
        await plugin.openUrl(url2, true);
        n = n + 1;
      } else {
        break;
      }
    }
    views = this.get_webviews(url2);
  ...
  async set_homepage(url2 = this.homepage, idx = 0) {
    this.view = await this.open_homepage(url2, idx);
  }
